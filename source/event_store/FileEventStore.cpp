...
void FileEventStore::append(const std::string &message) {
    if (out_fs_.is_open())
        out_fs_ << message << std::endl;
}

void FileEventStore::append(const IDomainEvent &event) {
    if (out_fs_.is_open())
...
std::string FileEventStore::getEventTypeFromJson(const std::string &json_str) {
...
std::vector<std::unique_ptr<IDomainEvent>> FileEventStore::getAllEvents() {
